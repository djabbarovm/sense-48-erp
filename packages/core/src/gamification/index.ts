 ...
 *
 * Принцип (уточнён по обратной связи владельца): XP не начисляется ни за «клики» (создать задачу,
 * переключить статус, открыть карточку), ни за сам факт звонка — звонок это попытка, а не результат.
 * XP выводится только из зафиксированных полезных РЕЗУЛЬТАТОВ: выполненный follow-up, проведённый
 * показ, отправленный оффер, закрытая сделка. Идемпотентность — по id факта: одно бизнес-событие
 * даёт XP один раз (см. db/services/gamification). Все пороги, XP и миссии — здесь, единым конфигом,
 * чтобы менять без правки CRM-логики (ТЗ §20).
 *
 * Уровни — recognition-слой, НЕ права (ТЗ §17): доступ определяет только RBAC. Геймификация не
 * инструмент управления людьми: владелец видит факты и нагрузку команды, а не «очки» сотрудников.
 */

export type GameEvent =
  | 'FOLLOW_UP_DONE' // выполненный follow-up
  | 'VIEWING_CONDUCTED' // показ проведён
 ...
 * Звонка здесь намеренно нет — за попытку дозвониться XP не дают, только за результат.
 */
 ...
 * и не «сделал N звонков»). Порог намеренно требует результата: один follow-up его не даёт,
 * поэтому streak отражает рабочие дни с реальной отдачей, а не присутствие или активность вхолостую.
 */
...
// «Передать лида» намеренно НЕ миссия: передача — служебный шаг, а не цель дня, и превращать
// её в KPI значит поощрять сбрасывание сырых лидов менеджеру ради галочки.
