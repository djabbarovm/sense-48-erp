...
import type { PropertyOwner, WorkOrderCategory } from '@prisma/client';
...
export async function getOwnerPortal(ctx: TenantContext, today = new Date()) {
  ...
  const documents = await prisma.document.count({ where: { tenantId: ctx.tenantId, OR: [{ objectType: 'lease_contract', objectId: { in: views.map((v) => v.lease?.id).filter((x): x is string => !!x) } }, { objectType: 'unit', objectId: { in: units.map((u) => u.id) } }] } });
  return {
