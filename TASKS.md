...
- [ ] P-30a Модель реестра ORDO (ADR-035): ось владения — enum `tenureStatus` (передан кадастр / ждёт выдачи / у компании / бартер / проблемный / судебный) на Unit + миграция; четыре независимые оси (владение / использование / аренда / продажа) на карточке и в фильтрах; типы PARKING и STORAGE как полноценные объекты
- [ ] P-30b Импортёр реестра Довлета и Озде (ADR-034/035): проход 1 — помещения Tower/Mall (кадастр. площадь, статус владения, без контактов); проход 2 — собственники c дедупликацией по телефону/имени (BR-P55) после сверки исходника; all-or-nothing, отчёт расхождений кадастр↔проект
- [x] P-30 Реальный тенант ORDO в bootstrap: `ordo` («ООО «ORDO Management»») создаётся без демо-данных, владелец OWNER+ADMIN; пилот Tower по системному логину — umar (CALL_CENTER), aziz (COMMERCIAL_MANAGER), email пуст, пароль через ротацию (APP_TEMP_PASSWORD); реестр помещений пуст, импорт отдельно (P-30b). Идемпотентно, проверено на dev: bootstrap создаёт `ordo` («ООО «ORDO Management»») без демо-данных, владелец OWNER+ADMIN; загрузка реального реестра помещений и собственников через `inventory.xlsx` (шаблон есть, P-07) и контактов (`contacts.xlsx`, P-22a); финансы ORDO — родными выгрузками 1С/Didox (H-09). Демо-тенант `piramit` остаётся синтетическим для показов. Блокер: реестр помещений/собственников и выгрузки 1С по ORDO от владельца
...
- [ ] P-09 Владелец: переименовать репозиторий и workspace-scope `@finance-os/*` в нейтральное имя + имя БД `finance_os` (DATABASE_URL, docker-compose), поле «Отправитель» обмена 1С, спец-доки (CLAUDE.md, docs/01/13/16/17, MASTER_PROMPT) — согласовано в ADR-036; git-история сохраняется
...
- [x] P-31 Сотрудники без email (системный логин): `User.email` nullable + `User.username` unique (миграция `user_username_optional_email`); вход по логину или email (`auth-actions`), форма входа принимает логин; сервис `createUser` (право user.manage, логин+временный пароль, роль в тенанте, аудит без пароля) + форма «Создать сотрудника» в /admin; авторизация через логин + временный пароль, дальше привязка Telegram; тесты. Пилот Tower: Umar Nazarov (CALL_CENTER), Muhammadaziz Gulomov (COMMERCIAL_MANAGER)
...
- [x] P-32 Нейминг: пользовательское «Finance OS» → «DMS» (UI: layout/login/sidebar, i18n, тема уведомлений, README, e2e); Property Assistant не тронут; технические идентификаторы (@finance-os/*, БД finance_os, обмен 1С, спец-доки) — список в ADR-036, переименование в P-09. ADR-036

- [x] P-33 Геймификация MVP (ТЗ §13-20, ADR-037): ядро (XP-правила/уровни/миссии по ролям/streak конфигом), db-слой (состояние из фактов, идемпотентно, tenant/role изоляция), награды (RewardRedemption + каталог из settings), блок прогресса и наград в «Мой день»; тесты ядра и сервиса. Осталось: точные метрики миссий (сейчас прокси), лидерборд (опц.), UI решения по заявкам наград для owner
- [x] P-34 Security-проход (ТЗ §28): проверены client-бандл секреты (нет), IDOR на deal/unit (whereTenant scoped), auth в server actions, Telegram webhook secret + изоляция chatId→user→tenant, публичное КП по токену (by design). Критических находок нет
- [x] P-32b Нейминг (добор): марка логотипа в сайдбаре и на входе выводила «FinanceOS» в обход i18n → «DMS». Пользовательских упоминаний «Finance OS» в apps/web не осталось
- [x] P-33b Геймификация — ревизия по обратной связи владельца (ADR-037 доп.): XP только за результат (звонок больше не даёт XP), убрана миссия «передать N лидов», streak-порог выше одного follow-up, удалён мёртвый код (события CALL_LOGGED/VIEWING_SCHEDULED/LEAD_QUALIFIED, метрика handoffs); core+db тесты зелёные
- [x] P-34b Security hardening: экраны `/pr` `/vendors` `/contracts` не имели верхнеуровневого guard на чтение (открывались по прямой ссылке в обход меню; данные tenant-scoped, мутации в сервисах защищены) → добавлен `notFound()` по праву меню, как на `/payments`. `/onec` `/migration` уже закрыты; `/approvals` `/portfolio` намеренно открыты и self-scoped
- [x] P-36 Role-based navigation (ADR-039): плоское меню из 44 пунктов сгруппировано по разделам; узкие роли получают компактный «Главное» + сворачиваемые «Все разделы»; нижнее меню на телефоне под роль. Права не изменены (презентация), проверено e2e (focused vs broad) на живом сервере. i18n `nav.section.*`. Осталось: тюнинг PRIMARY_NAV, подписи ролей текстом
- [x] P-35 Роли COMMERCIAL_DIRECTOR (Kamila) и CEO как операционные профили (ADR-038): director = гранты COMMERCIAL_MANAGER (область видимости — в домене); CEO = только видимость (все `*.view` + дашборды + аудит), без изменения/согласования (four-eyes цел, зафиксировано тестами); core `ROLE_CODES` + Prisma enum + миграция `20260919180000_roles_director_ceo`. Осталось: роль-ориентированная навигация (сборка меню под профиль), человеко-читаемые подписи ролей в админке
...
- [~] TWR-02 Phase 2: воронка сделки §3.1 (forward-only + откат COMMERCIAL_DIRECTOR, DUE_DILIGENCE-гейт перед CONTRACT — миграция enum), экран CALL_CENTER «Очередь входящих» (§5.1), приведение доски BROKER (§5.2), расширение BROKER до полного цикла
  ...
  - [x] 2b: §1-корректива ТЗ (у BROKER убран rent.view — брокеридж не ведёт дебиторку, ADR-043); применены M1 (DUE_DILIGENCE enum) + M2 (tower_setting) + сервис порогов (live-edit, policy.manage). 543 зелёных
  - [ ] 2c: сущности Intent + лёгкий Mandate (гейт WARN) — ОТДЕЛЬНАЯ миграция (показать+откат); воронка §5.1 forward-only + откат=DIRECTOR; гейт DUE_DILIGENCE перед CONTRACT; экран CALL_CENTER «Очередь входящих»; доска сделок BROKER
    - [ ] Дополнение №1 (ADR-044): на видах фонда (2.5D/шахматка/план этажа) по юниту показать ставку+срок аренды+арендатора из хранимого Contract/Listing (зеркало на Unit), НЕ из rent-ledger — в этой же фазе экранов
- [ ] TWR-03.5 Паркинг-пул (ТЗ §5.3/§6.2): инвентарь ~300 мест, тарифы, сдача, сбор аренды, дебиторка, 15%, отчёт девелоперу — сюда переносится rent-charges/receivables (на LTR не запускать)
