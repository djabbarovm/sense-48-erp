...
- [ ] P-23 CRM: «Мой день» `/me` mobile-first, PWA, нижняя навигация, быстрые действия (docs/21 §5, §8)
- [ ] P-24 CRM: Telegram-бот — webhook, привязка `/start`, интенты §6, кнопки, уведомления и дайджесты (docs/21 §6); блокер: токен бота (mock до получения)
- [ ] P-25 CRM: роль CALL_CENTER, BR-P59…P62 (SLA лида, показ как событие, нет сделки без шага, КЦ до показа)
- [ ] P-26 CRM: аналитика (`/crm/analytics`), `TelephonyAdapter` + журнал звонков (docs/21 §7)
