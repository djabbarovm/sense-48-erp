...
- [!blocked] P-14 Бонусы продажников (blueprint §17): нужны положение о бонусах — ставки, пороги, правила удержания (docs/16 вопрос 14). Модель: потенциальный → подтверждённый → к выплате → удержан; база = маржинальный доход × ставка
...
- [~] P-18 Аренда и дебиторка (blueprint §1.7 Finance, §1.8 «c задолженностью», §9 Finance): RentCharge по месяцам c пропорцией (BR-P38), PAID только зачётом банковской транзакции (BR-P37), списание будущих периодов при прекращении (BR-P39), просрочка → Task RENT_OVERDUE + событие, фильтр «c задолженностью», блок Finance в карточке юнита и на пульте, `/rent` c зачётом поступлений, выписка собственника из начислений; джобы rent-charges/rent-overdue; 2 core + 4 db теста. Осталось: seed (счёт + поступления), проверка в браузере, docs/20 §11.9, ADR
