...
| 5c CRM | «Мой день» mobile-first, PWA, быстрые действия, показ как событие ✓ P-23 | ✓ |
| 5d CRM | Telegram-бот сотрудника: привязка, намерения, кнопки, напоминания, SLA лида, дайджесты ✓ P-24 | ✓ |
| 5e CRM | Аналитика CRM (скорость, объём, конверсия, качество, звонки) и адаптер телефонии ✓ P-26 | ✓ |
| 5f CRM | Коммерческое предложение по ссылке: подборка, просмотры, запрос показа, уведомления ✓ P-28 | ✓ |
...
### 11.10 Комиссии ORDO и бонусы продажников (blueprint §17; бизнес-модель Tower §2; решения владельца 19.09.2026)
Продукт сделки (`Deal.product`: LEASE_LTR, LEASE_OFFICE, SALE, STR_MANDATE, PARKING_LEASE, PARKING_SALE, MALL_LEASE) задаёт правило комиссии ORDO — плательщик, база, ставка (`COMMISSION_RULES`): аренда квартиры/офиса — 50% месяца c арендатора; продажа — 3% c продавца, коридор 1,5–3% (переопределение только внутри коридора); частный паркинг — 50% месяца c собственника места; STR и ТРЦ — ставка не утверждена (OPEN): сделка выигрывается, комиссия не начисляется до решения. `Commission` (CM-номера) начисляется на WON — при активации договора (база = месячная аренда) или закрытии продажи ценой (`closeSale`); доля внешнего брокера (`externalShareBp`) уменьшает чистую комиссию (база бонуса и выручка ORDO). PAID — только зачётом входящей банковской транзакции (`ReconciliationMatch` COMMISSION).

Бонус продажника (`SalesBonus`): аренда — 20% чистой комиссии за сделку (CONFIRMED на WON) + 10% за KPI-чек-лист после заезда (онбординг, ключи доступа, интернет, клининг, передача в Services; подтверждает коммерческий менеджер, не продажник; срок `bonus_kpi_deadline_days` = 14 дней от старта договора; просрочка → WITHHELD джобом `bonus-kpi-deadline`); продажа — треть комиссии (`bonus_sale_deal_bp` = 3333 ≈ 1% от суммы при 3%); STR — не решено, бонус не создаётся. К выплате (PAYABLE) — только когда комиссия PAID; финансы отмечают выплату по ведомости (PAID, период YYYY-MM, номер) без автопроводки в зарплату. Экраны: `/commissions` (сводка, зачёт поступлений), `/bonuses` (отчёт за период по сотрудникам, выплата; продажник видит только свои), блок «Комиссия ORDO и бонус» c чек-листом в карточке сделки, продукт/цена продажи/внешний брокер в формах сделки.

| ID | Правило | Поведение | Тест |
|---|---|---|---|
| BR-P41 | Комиссия по правилу продукта, только на WON | плательщик/база/ставка из COMMISSION_RULES; ставка вне коридора → COMMISSION_RATE_OUT_OF_CORRIDOR; OPEN-ставка → без комиссии | commission.test, commissions.test |
...
### 11.13 Operations — деньги дома: взносы по кадастру, фонд, прозрачность (бизнес-модель Operations v1.0; ЗРУ-581)
Фонд дома (`HouseFund`: OPERATIONS / MARKETING / CAPEX) привязан к зданию и к **отдельному счёту дома**; тариф за м² в месяц, день оплаты (по акту от 05.02.2026 — до 15 числа), дата утверждения тарифа собранием, ставка вознаграждения ORDO (`managementFeeBp`, **OPEN** до заключения Quantum Law — пока `null`, строка вознаграждения собственникам не публикуется). Взнос собственника = кадастровая площадь × тариф (`Unit.cadastralNumber / cadastralAreaM2`); пока кадастр не внесён — договорная площадь c пометкой `preCadastre`. Джоб `house-charges` (1-го числа) создаёт `HouseCharge` (HC-номер) по всем жилым/коммерческим юнитам здания фонда; юниты без собственника попадают в отчёт «без плательщика». Джоб `house-overdue` — OVERDUE + задача HOUSE_CHARGE_OVERDUE (дедуп по собственнику) + событие. PAID — только зачёт поступления **на счёт дома** (`matchHouseReceipt`; поступление на счёт ORDO → WRONG_ACCOUNT). Бюджет дома по статьям на год (`HouseBudgetLine`), расходы из денег дома (`HouseExpense`: подрядчик, статья, основание — документ / заявка на оплату); LIFTS, FIRE_SAFETY, ENGINEERING — compliance floor. Дашборд `/house`: начислено/собрано/собираемость/просрочено/потрачено, вознаграждение отдельной строкой, годовой перерасчёт (ст. 16: остаток → кредит собственникам, а не прибыль), сценарии 70/85/95%, должники, плательщики, охват договоров управления (ст. 28: `PropertyOwner.managementContractStatus`). Отчёт собственникам за месяц (ст. 29) «деньги · работы · качество · дальше» — `/house?view=report` и кабинет собственника (свои взносы, остаток).

| ID | Правило | Поведение | Тест |
|---|---|---|---|
| BR-P51 | Взнос = кадастровая площадь × тариф | без кадастра — договорная площадь c пометкой preCadastre; технические/общие юниты и юниты без собственника не начисляются; идемпотентно по (юнит, месяц) | house.test, house (core) |
| BR-P52 | Деньги дома ≠ деньги ORDO | фонд c отдельным счётом; зачёт поступления не на счёт дома → WRONG_ACCOUNT; вознаграждение ORDO — отдельная строка, при OPEN-ставке не публикуется | house.test |
| BR-P53 | PAID только банком | статус взноса меняется только зачётом банковской транзакции (ReconciliationMatch HOUSE_CHARGE); UI-кнопки «оплачено» нет; частичная оплата → PARTIAL | house.test |
| BR-P54 | Годовой перерасчёт | собрано − потрачено > 0 → кредит собственникам в следующий период, < 0 → недобор; план/факт по статьям, compliance floor | house.test, house (core) |

### 11.14 CRM Tower — клиентская база (P-22a)
`Contact` (kind PERSON/COMPANY, displayName, phone канонический `+digits`, phoneAlt, email, company, position, source, tags[], notes, ownerId → PropertyOwner, managerId, lastTouchAt). Сделка (`Deal.contactId`) и лид привязываются к контакту автоматически: телефон → доп. телефон → email; собственник c тем же телефоном — тот же человек. Экраны: `/contacts` (поиск по имени/компании/телефону/email, менеджер, только собственники, счётчики сделок, последняя стадия, давность контакта), `/contacts/[id]` (контакты, сделки, история активностей, блок собственника, правка, слияние дублей, «Новая сделка» c предзаполнением), ссылка на клиента в карточке сделки, импорт `contacts.xlsx` на `/migration`. PII (телефон/email) — только c `deal.contact.view`, иначе маска `+99890***4567` / `a***@mail.uz`; в audit PII не пишется.
...
### 11.16 CRM Tower — «Мой день» и мобильная версия (P-23; docs/21 §5, §8)
`/me` — личный экран сотрудника (mobile-first, PWA): итог дня (звонки, показы, лиды, в работе), показы сегодня c результатом одним действием (оффер / думает / перенос / отказ c причиной), новые лиды, просроченные и без следующего шага, сегодня по плану, собственники на связь, задачи. Быстрые действия через сервисы сделок/собственников: звонок + следующий шаг, назначить показ (BR-P60: активность VIEWING c датой и временем, сделка → VIEWING, следующий шаг «провести показ»), быстрый лид (NEW, следующий шаг «связаться» через 15 минут — база для SLA BR-P59), звонок собственнику, задача сделана. Менеджер и брокер видят своё, OWNER/ADMIN/FINANCE_OPS_LEAD — команду. Нижняя навигация на ширине < 768; менеджер и брокер стартуют c `/me`.

### 11.17 CRM Tower — Telegram-бот сотрудника (P-24; docs/21 §6)
Привязка чата кодом из «Мой день» (`/start <код>`, один чат — один сотрудник, код одноразовый). Бот действует от имени сотрудника: контекст — тенант c его CRM-ролью, права и аудит его. Свободный текст → `createActionDraft(source TELEGRAM)` → preview c кнопками «Подтвердить / Отмена» → `confirmActionDraft` (BR-P30); запросы (мой день, список юнитов) выполняются сразу. Команды: `/today` (дайджест), `/leads`, `/deal <юнит|номер>` (c кнопками результата, если показ), `/owners`, `/help`. Пересланное сообщение клиента → лид. Напоминания (`crm-reminders`, каждые 15 минут): показ за час → менеджеру; показ прошёл 2 часа назад без активности → «Как прошёл показ?» c кнопками (оффер / думают / перенос / отказ → причина); лид NEW без касания сотрудника 15 минут → менеджеру, 30 минут → руководителю, только 09:00–20:00 Ташкента. Дайджесты: 08:30 план дня, 19:00 итог и завтра. Дедупликация — маркеры `DomainEvent` (`viewing.reminder`, `viewing.result.prompt`, `lead.sla.warn`, `lead.sla.escalate`).
...
### 11.18 CRM Tower — аналитика и телефония (P-26; docs/21 §7)
`/crm/analytics` (deal.view): скорость (медиана и среднее первого касания лида сотрудником, доля в SLA 15 минут, без касания — по источникам), конверсия шагов (лид → показ → оффер → договор), активности по видам и в день, по сотрудникам (лиды, звонки c пометкой телефонии, показы, офферы, заметки, касания собственников, среднее первое касание, в работе, выигр./проигр.), качество (без следующего шага, просрочено, без движения 7 дней, показы без результата, SLA-предупреждения и эскалации), звонки (внесённые / из телефонии, входящие, исходящие, пропущенные, средняя длительность), собственники (движения, подписано, просрочено). Телефония — `TelephonyAdapter` + `POST /api/telephony/webhook` (scope TELEPHONY) → `ingestCallEvent` (docs/07 §8); поля `UnitActivity.durationSec / recordingUrl / callDirection / externalRef`, source `PHONE`.

### 11.19 CRM Tower — коммерческое предложение (P-28; docs/21 §3)
`DealProposal`: до 6 помещений, токен ссылки, комментарий, срок действия (по умолчанию 7 дней), просмотры (`viewsCount`, `viewedAt`), запрос показа (`viewingRequestedAt`, `requestNote`), отметки уведомлений. Создание — `deal.manage` по своей сделке; активность OFFER «КП отправлено» и следующий шаг «узнать реакцию» через 2 дня. Публичная страница `/p/<token>` без входа: компания, имя менеджера, срок, помещения (номер, здание, этаж, тип, площадь, ставка, готовность) — без собственника, арендатора и телефонов; печать в PDF; «Записаться на показ» (один раз) → активность FOLLOW_UP, следующий шаг «назначить показ» сейчас. Первое открытие и запрос показа приходят менеджеру в Telegram (crm-reminders). Истёкшее КП не считает просмотры и не принимает запрос.
