...
### Сервер DigitalOcean по кнопке (H-06/H-11)
...
- **Телефония OnlinePBX.** «Администрирование → API-ключи» → ключ co scope TELEPHONY; «Администрирование → Телефония» → провайдер OnlinePBX, зона АТС, внутренние номера сотрудников («101 = email»). В кабинете OnlinePBX: Сервисы → Интеграция → Webhooks → адрес `$APP_URL/api/telephony/webhook` (заголовок `X-Api-Key`, либо `?key=…` в адресе), события «Завершили» и «Пропущенный». После первого звонка проверьте в «Телефонии» строку «Последний webhook»; если тело не распознано — там же показаны его ключи, и карта полей уточняется в поле JSON (docs/07 §8).
